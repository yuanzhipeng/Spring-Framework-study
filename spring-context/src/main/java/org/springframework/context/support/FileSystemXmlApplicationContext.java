...
public class FileSystemXmlApplicationContext extends AbstractXmlApplicationContext {
	 ...
	 * @see #refresh()
	 *
	 * 在对象初始化的过程中，调用refresh()函数载入BeanDefinition，这个refresh启动了
	...
	public FileSystemXmlApplicationContext(
		...
		if (refresh) {
			// refresh()标志着IoC容器的正式启动
			// 启动包括BeanDeefinition的Resource定位、载入和注册三个基本过程
			refresh();
		}
	}
	 ...
	 *
	 * 这是应用于文件系统中Resource的实现，通过构造一个FileSystemXmlResource来得到一个在文件
	 * 系统中定位的BeanDefinition
	 * 这个getResourceByPath是在BeanDefinitionReader的loadBeanDefinition中被调用的
	 * loadBeanDefinition采用了模版模式，具体的定位实现实际上是由各个子类来完成的
	 */
	@Override
