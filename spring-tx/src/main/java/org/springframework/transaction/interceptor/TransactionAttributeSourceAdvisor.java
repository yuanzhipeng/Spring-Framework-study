...
public class TransactionAttributeSourceAdvisor extends AbstractPointcutAdvisor {
	...
	//与其他Advisor一样,同样需要定义AOP中用到Interceptor和pointcut
	//Interceptor使用的是已经见过的拦截器:TransactionInterceptor
	@Nullable
	private TransactionInterceptor transactionInterceptor;

	//对于pointcut,这里定义了一个内部类TransactionAttributeSourcePonitcut
	private final TransactionAttributeSourcePointcut pointcut = new TransactionAttributeSourcePointcut() {
		@Override
		@Nullable
		protected TransactionAttributeSource getTransactionAttributeSource() {
			//这里通过调用transactionInterceptor来得到事务的配置属性,在对Proxy的方法进行匹配调用时,
			//会使用到这些配置属性
