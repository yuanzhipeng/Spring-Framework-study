...
public class DefaultAdvisorAdapterRegistry implements AdvisorAdapterRegistry, Serializable {
	...
	//这里是在DefaultAdvisorChainFactory中启动getInterceptors方法
	@Override
	public MethodInterceptor[] getInterceptors(Advisor advisor) throws UnknownAdviceTypeException {
		List<MethodInterceptor> interceptors = new ArrayList<>(3);
		//从Advisor通知器配置中取得advice通知
		Advice advice = advisor.getAdvice();
		//如果通知是MethodInterceptor类型的通知,直接加入interceptors的
		//List中,不需要适配
		if (advice instanceof MethodInterceptor) {
			interceptors.add((MethodInterceptor) advice);
		}
		//对通知进行适配,使用已经配置好的Adapter:MethodBeforceAdviceAdapter、
		//AfterReturningAdviceAdapter以及ThrowsAdviceAdapter,
		//然后从对应的adapter中去除封装好的AOP编织功能的拦截器
		for (AdvisorAdapter adapter : this.adapters) {
			if (adapter.supportsAdvice(advice)) {
