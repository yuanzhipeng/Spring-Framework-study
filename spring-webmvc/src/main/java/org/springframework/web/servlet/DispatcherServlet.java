...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		//这里导入所有的HandlerMapping Bean,这些Bean可以在当前的DispatcherServlet的
		//IoC容器中,也可能在其双亲上下文中
		if (this.detectAllHandlerMappings) {
			...
			//也可以根据名称从当前的Ioc容器中通过getBean获取handlerMapping
			try {
		...
		//如果没有找到handerMappings,那么需要为Servlet设定默认的handlerMappings,这些默
		// 认的值可以设置在DispatcherServlet.properties中
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				//这里实际调用handler的地方,在执行handler之前,用H按到了让A搭配特人先检查一下
				//handler处理的合法性:是不是按Spring的要求编写的handler
				...
				//通过调用handlerAdapter的handler方法,实际上触发对Controller的handleRequest方法的调用
