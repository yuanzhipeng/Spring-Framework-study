...
public abstract class DataSourceUtils {
	...
	public static Connection doGetConnection(DataSource dataSource) throws SQLException {
		...
		/* 把对数据库的Connection 放到事务管理中进行管理,这里使用在Transaction-
		SynchronizationManager中定义的ThreadLocal变量来和线程绑定数据库连接 */
		/* 如果在SynchronizationManager中已经有了与当前线程绑定的数据库连接,那就直接取出来使用 */
		...
		//最后把新打开的数据库Connection通过TransactionSynchronizationManager和当前线程
		//绑定起来
